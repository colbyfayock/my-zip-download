...
const Button = ({ children, className = '', color = 'slate', href, onClick }: ButtonProps) => {
  ...
  if ( href ) {
    return (
      <a href={href} className={`inline-block rounded py-2.5 px-6 text-sm font-bold uppercase ${buttonColor} ${className}`}>
        { children }
      </a>
    )
  }
